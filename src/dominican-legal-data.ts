...
export interface LegalProfession {
  requirements: string;
  regulation?: string;
  functions?: string;
  selection?: string;
...
export interface TimeFrames {
  civil_actions: string;
  labor_claims: string;
  criminal_cases: string;
}

export interface DeadLines {
  civil_response: string;
  civil_appeal: string;
  criminal_appeal: string;
  labor_appeal: string;
}

export interface DominicanLegalData {
  legal_system: {
    overview: string;
    court_hierarchy: Court[];
  };
  primary_legislation: LegalCode[];
  important_laws: Law[];
  legal_procedures: {
    civil_procedure: {
      ordinary_process: string[];
      appeals: string[];
      enforcement: string[];
    };
    criminal_procedure: {
      investigative_phase: string[];
      trial_phase: string[];
      appeals: string[];
    };
    labor_procedure: {
      conciliation_phase: string[];
      judicial_phase: string[];
      appeals: string[];
    };
  };
  common_legal_issues: {
    property_disputes: LegalIssue;
    family_matters: LegalIssue;
    labor_disputes: LegalIssue;
    business_issues: LegalIssue;
  };
  legal_professions: {
    attorneys: LegalProfession;
    notaries: LegalProfession;
    judges: LegalProfession;
  };
  important_legal_timeframes: {
    statute_of_limitations: TimeFrames;
    procedural_deadlines: DeadLines;
  };
}

// Export the legal data
export const dominicanLegalData: DominicanLegalData = {
  legal_system: {
